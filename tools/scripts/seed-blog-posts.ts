...
const authorId = "eb81de5e-a088-4be4-8fed-5540d9fbb37c";

const post1Content = `## Why you need an approval step
...
Sign up at [okrunit.com](https://okrunit.com). The free tier gives you 100 requests/month, which is plenty for getting started.
...
OKrunit's free tier includes 100 requests/month with 2 connections. No credit card required. [Sign up at okrunit.com](https://okrunit.com/signup)`;

const post2Content = `## The autonomy problem
...
This is not about making agents less useful. It is about making them safer. Most agent actions are fine to execute automatically. But for the subset that involves:
...
The key is that the agent provides context: what it wants to do, why it wants to do it, and what data is affected. This lets the human make an informed decision without needing to agent's full internal state.
...
Free tier, no credit card required. [Sign up at okrunit.com](https://okrunit.com/signup)`;

const post3Content = `Automations are incredibly powerful. They are also incredibly dangerous when they go wrong. Here are five real-world scenarios (names changed) where a simple "are you sure?" step would have saved the day.
...
OKrunit works with Zapier, Make, n8n, GitHub Actions, and any tool that can make an HTTP request. Free tier: 100 requests/month. [Sign up at okrunit.com](https://okrunit.com/signup)`;

const posts = [
  {
    slug: "how-to-add-human-approval-to-zapier",
    title: "How to Add Human Approval to Any Zapier Zap",
    description: "Step-by-step guide to pausing your Zapier Zaps for human review before they execute critical actions like bulk deletes, email sends, or production deploys.",
    category: "Guides",
  ...
  {
    slug: "why-ai-agents-need-human-in-the-loop",
    title: "Why Every AI Agent Needs a Human-in-the-Loop",
    description: "AI agents are getting more autonomous every month. Here is why inserting a human decision point before high-risk actions is not a bottleneck, it is a safety net.",
    category: "AI Safety",
  ...
  {
    slug: "automation-disasters-prevented",
    title: "5 Automation Disasters That Could Have Been Prevented with One API Call",
    description: "Real-world scenarios where a simple human approval step would have saved hours of cleanup, thousands of dollars, and a lot of stress.",
...
async function main() {
  const { data, error } = await s.from("blog_posts").insert(posts).select("id, slug, title, published");
  if (error) {
  ...
  for (const p of data) {
    console.log(` - ${p.title} (/blog/${p.slug}) [published]`);
  }
}
