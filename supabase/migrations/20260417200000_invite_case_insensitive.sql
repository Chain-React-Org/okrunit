-- Fix case-insensitive email matching in invite lookup.
-- The invite page normalizes emails with toLowerCase() but the trigger
-- was doing a case-sensitive comparison, causing mismatches when the
-- invite email and signup email differed in casing.
...
BEGIN
  ...
  -- Check if user was invited to an existing org (case-insensitive)
